  ...
  methods:{
    ...
    save(){
      if (this.form.albumId){
        request.put("/album",this.form).then(res =>{
          console.log(res)
          if (res.code==='0'){
            this.$message({
              type:"success",
    ...
    handleDelete(userId){
      console.log(userId)
      request.delete("/album/" + userId).then(res=>{
        if (res.code==='0'){
          this.$message({
            type:"success",
            message:"删除成功"
